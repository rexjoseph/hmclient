...
const EmailV1SubscribersList = () => {
  const [subscribers, setSubscribers] = useState([]);
  const [loading, setLoading] = useState(false);
  const dispatch = useDispatch();

  useEffect(() => {
    document.title = `Admin Email v1 Subscriber List — Hashingmart`;
  });

  useEffect(() => {
    const getSubscribers = async () => {
      try {
        const res = await userRequest.get('/email/v1/all')
        ...
        <div className="flex">
          ...
          <div className="mainbody">
            <div className="mainbody-header">
              <h1 className="mainbody-title">Email subscribers (form)</h1>
              <div className="span">
                <span>
...
export default EmailV1SubscribersList